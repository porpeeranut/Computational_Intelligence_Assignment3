...
public class GA {
	...
	public void train(int maxGeneration, ArrayList<Double[]> trainingSet){
		...
		for (int g = 0;g < maxGeneration;g++) {
			...
			int mutateAmount = (int) (chromosomeList.size()*0.4);
			ArrayList<Chromosome> mutated = mutate(pool, mutateAmount);
			//pool.addAll(mutated);
			...
			chromosomeList = new ArrayList<Chromosome>(pool.subList(0, chromosomeList.size()-mutateAmount));
			//chromosomeList = new ArrayList<Chromosome>(pool.subList(0, chromosomeList.size()));
			chromosomeList.addAll(mutated);
			System.out.println("Generation "+(g+1)+" best accurate: "+(chromosomeList.get(0).fitness*100/trainingSet.size())+"%");
		}
		...
		for (Chromosome chromosome : chromosomeList) {
			//System.out.println(chromosome.fitness+" "+(chromosome.fitness*100/trainingSet.size()));
		}
	}
	
	public double test(ArrayList<Double[]> testSet){
		MLP mlp = new MLP(MLP_struct);
		Chromosome bestChomos = chromosomeList.get(0);
		mlp.setWeightFromChromosome(bestChomos);
		bestChomos.fitness = 0.0;
		for (Double[] data : testSet) {
			if (mlp.computeForward(data))
				bestChomos.fitness += 1;
		}
		//computeFitness(chromosomeList.get(0));
		System.out.println("Testing accurate: "+(bestChomos.fitness*100/testSet.size())+"%");
		return 100-(bestChomos.fitness*100/testSet.size());
	}
